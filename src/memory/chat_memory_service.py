...
from database.repository.meta.user_history_repository_meta import (
    UserHistoryRepositoryMeta,
)
...
class ChatMemoryService:
    ...
    async def add_chat_message(
        self, user_id: UUID, user_input: str, response_content: str
    ):
        """Add chat message using existing repository"""
        return await self.user_history_repo.add_one(
            user_input, response_content, user_id
        )
    ...
    async def add_procedural_memory(self, skill_name: str, steps: str):
        ...
        memory = ProceduralMemory(skill_name=skill_name, steps=steps)
        self.session.add(memory)
        await self.session.commit()
        return memory
